...
class Database:
    ...
    def search(self, cam=None, color=None, license=None, time=None):
        ...
        for row in rows:
            ret.append({
                ...
                KEYS.CARIMAGE: os.path.join(BASE_DIR, "car_images", str(row[6])) if row[6] else "",
                KEYS.LICENSENUMBER: row[5],
                KEYS.LICENSEIMAGE: os.path.join(BASE_DIR, "license_images", str(row[4])) if row[4] else "",
    ...
    def delete_violation(self, carid, ruleid, time):
        cur = self.con.cursor()
        if time is None:
            command = "update violations set cleared = true where car = ? and rule = ? and time is NULL"
            cur.execute(command, (carid, ruleid))
        else:
            command = "update violations set cleared = true where car = ? and rule = ? and time = ?"
            cur.execute(command, (carid, ruleid, time))
